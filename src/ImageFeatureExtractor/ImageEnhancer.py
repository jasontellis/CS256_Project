...
class ImageEnhancer:
	...
	def __init__(self, imageFileName, facecascxml_path, eyecascxml_path, ref_image_feature =[1,125,]):
	#self.feature_vector = [face_num, img_face_brightness, img_face_hue, img_face_ratio, img_face_worstSNR,img_eye_signal, img_contrast, img_sharpness]
		self.ref_image_feature = ref_image_feature
	...
	def extract(self):
		...
		gray_pd=np.array(gray.histogram())/float((gray_stat.sum)[0])
		gray_std=np.sqrt(np.sum(np.multiply(np.square(np.array(range(256))+1-gray_mean),gray_pd)))
		gray_mode=np.where(gray_pd==np.max(gray_pd))[0]
		gray_sk2=(gray_mean-(gray_mode[np.ceil(len(gray_mode)/2)]+1))/float(gray_std)
		gray_eng=np.sum(np.square(gray_pd))
		...
		self.feature_vector = [face_num, img_face_brightness, img_face_hue, img_face_ratio, img_face_worstSNR,
		                       img_eye_signal, img_gray_ep, img_sharpness]
